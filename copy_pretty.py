...
class wave_2d:
    ...
    def __init__(self,Lx, Ly, T, Nx, Ny, dt, mx,b = 0,I_f=None, V_f=V_f_glob, q_f=q_f_glob, f_f=f_f_glob,exact=None, gauss=False, standing=False,plug=False,constant =False):
        """
    ...
    def solve_num(self):
        ...
        if self.exact != None:
            errorfil.write("error for dt=%4.2f and mx=%g : %6.4f \n"%(dt,self.mx, max(err)))
...
parser.add_argument("-mx", type = float, dest="mx", help="wavelength")
...
mx = args.mx if args.mx !=None else 3.0
...
w = wave_2d(Ly,Lx,T,Nx,Ny,dt,mx,b,standing=args.standing,gauss=args.gauss,plug=args.plug,constant = args.constant,q_f=g)
